...
export function useContactForm({ standalone, triggerRef }: UseContactFormProps) {
  ...
  const handleSubmissionSuccess = useCallback(async (response: Response, startTime: number) => {
    try {
  ...
  const handleSubmissionError = useCallback((errorData: ContactFormErrorResponse) => {
